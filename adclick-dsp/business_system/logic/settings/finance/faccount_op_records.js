...
var URLPATH = '/v3/settings/finance/recharge/list';
...
function packageResponseData(data){
    ...
    for (var i = 0; i < records.length; i++) {
        var record = {
            ...
            account_type: ADCONSTANTS.FINANCIALACCOUNT.format(records[i].account_type),
            amount: mUtils.fenToYuan(records[i].amount),
            charge_type: ADCONSTANTS.FINANCIALRECHARGE.format(records[i].charge_type),
            charge_status: ADCONSTANTS.REACHAGESTATUS.format(records[i].charge_status),
        }; 
...
function preprocess(param) {
    ...
    if (param.sort) {
        ...
        param.sort = ADCONSTANTS.DATASORT.UPDATETIME_DESC.code;
    }
}
