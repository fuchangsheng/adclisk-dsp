...
var URLPATH = '/v1/message/receivers/add';
...
var mSMSHelper = require('./sms_request');
...
function sendVerifyCodeByEmail(param, fn) {
    ...
    mAsync.series([
        ...
        function(next) {
            ...
            mEmailVerifyLogModel.update(query, function(err, data) {
                needNewCreate = (data.affectedRows == 0);
                next(err);
            });
        },
        //3. if effect row to update is 0, to create
        function(next) {
            if(!needNewCreate) {
                return next(null);
            }
...
function processRequest(param, fn){
    ...
    var logmsg = 'to add receiver for user:' + mgr_id+', receiver:'+receiver;
