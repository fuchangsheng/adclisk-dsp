...
var mRefModel = {
    ...
    categories: {
        data: '',
        rangeCheck: function(data) {
            var type = ADCONSTANTS.MANAGERMESSAGECATEGORIES.find(data);
...
function preprocess(param) {
    if (param.categories) {
        param.categories = ADCONSTANTS.MANAGERMESSAGECATEGORIES.parse(param.categories);
    }
    if (param.start_time) {
...
function commonSql(param) {
    var sqlstr =' where receiver="'+param.mgr_id+'"';
    if (param.categories!=ADCONSTANTS.MANAGERMESSAGECATEGORIES.ALL.code) {
        sqlstr += ' and (categories='+ param.categories+')';
    }
    if (mUtils.isExist(param.notify_status)) {
        sqlstr += ' and (notify_status='+param.notify_status+')';
    }
     //sqlstr +=' and ( subcategories IN ('+subcategories.join(',')+'))';
...
function processRequest(param, fn){
    ...
    mLogger.debug('Try to get the internal message for manager :' + mgr_id);
    ...
    function(err, result){
        if (err) {
            mLogger.error('Failed to get the msg list for user ' + mgr_id);
            fn(err);    
        }else{
            mLogger.info('Success to get the msg list for user ' +  mgr_id);
