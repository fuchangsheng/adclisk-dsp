/*
 * @file  admin list.js
 * @description admin list API and logic
 ...
 */
'use strict';
var MODULENAME = 'admin_list.logic';
var URLPATH = '/v1/admin/list';
...
function packageResponseData(data){
    ...
    for (var i = 0; i < administrators.length; i++) {
        var admin = {
            ...
            role: ADCONSTANTS.ROLE.format(administrators[i].role),
            phone: administrators[i].phone,
            email: administrators[i].email
        };
        resData.list.push(admin);
    }
    return resData;
}
...
function processRequest(param, fn){
    ...
    var logmsg = ' to get the mgr list in the system' ;
    mLogger.debug('Try '+logmsg);

    mAsync.waterfall([
        //1. check total number of admin in the system
        function(next) {
            var match = {};
            var query = {
                match: match,
            };
            mAdminModel.count(query, function(err, total) {
                if (err) {
                    next(err);
                }else {
                    next(null, total);
        ...
        //2. get the admin list
        function(total, next) {
            if (total==0) {
                return next(null, {total: 0, administrators: []});
            }
            ...
            if (offset>=total) {
                mLogger.info(
                    'There is no more admin in the system');
                return next(null, {total: total, administrators: []});
            }

            //2.1 create the sql statment
            var sqlstr = 'select * ' ;
            sqlstr += ' from ' + mAdminModel.tableName;
            ...
            mAdminModel.query(query, function(err, rows) {
                if (err) {
                    next(err);
                }else {     
                    next(null, {total: total, administrators: rows});
                }
            });
        },
    ], function(err, data) {
