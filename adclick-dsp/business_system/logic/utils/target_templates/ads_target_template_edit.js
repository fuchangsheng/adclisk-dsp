/*
 * @file  ads_target_template_edit.js
 ...
 */
'use strict';
var MODULENAME = 'ads_target_template_edit.logic';
var URLPATH = '/v3/utils/tt/edit';
...
function removeOldTargetTemplateContents(param, fn) {
...
function transactionCallback(param, fn) {
    ...
    mAsync.series([
        function(next) {
            removeOldTargetTemplateContents(param, next);
        }, 
        function(next) {
...
function processRequest(param, fn){
    ...
    var logmsg = ' to edit ad target template for user:' + user_id ;
    ...
    mAsync.series([
        ...
        function(next) {
            ...
            mADTargetTemplateModel.lookup(query, function(err, rows) {
                if (err) {
                    ...
                    if (rows.length > 0 && rows[0].template_id!=param.template_id) {
                            var msg = 'The idea name duplicated!';
                            mLogger.error(msg);
        ...
        function(next) {
            ...
            if(param.tag) {
                update.tag = param.tag;
            }
            var query = {
                match: match,
                update: update
            }
            mADTargetTemplateModel.update(query, next);
        },
        function(next) {
