...
var l_con = 3;
var r_con = 5;
var type = ADCONSTANT.OVERVIEWTYPE.USER;

$(function() {
    var tabs_config = [ {
        "name" : "overview_all",
        "text" : "Overview",
        "url" : "/overview_all.html"
    }, {
        "name" : "overview_adx",
        "text" : "ADX数据",
        "url" : "/overview_adx.html"
    }, {
        "name" : "over_view",
        "text" : "广告主数据",
        "url" : "/overview_user.html"
    } ];
    var current_tab = window.current_tab || "";
    initTabs(tabs_config, current_tab);
    if (current_tab == "overview_user") {
...
function selectchange() {
    ...
    l_con = $('#l_con').val();
    r_con = $('#r_con').val();
    initChart(myChart, l_con, r_con, arr[0], arr[1], choosed_user_id, type);
}

function initTable(index, count, start_time, end_time) {
    var tid = "#overview_table";
...
$('#downloadcsv').click(function() {
    ...
    param = {
        sinterface : SERVERCONF.DASHBOARD.OVERVIEWUSERDOWNLOAD,
        data : {
            user_id : choosed_user_id,
            start_time : arr[0],
            end_time : arr[1],
