...
ffm_int  my_ffm_save_model(ffm_model *model, char const *path){
    FILE *fp = fopen(path, "w");
    printf("ok new ffm save model\n");
    fprintf(fp, "n %d\nm %d\n k %d\nnormalization %d \n", static_cast<int>(model->n), static_cast<int>(model->m), static_cast<int>(model->k), static_cast<int>(model->normalization));

    ffm_float *ptr = model->W;
    for(ffm_int j = 0; j < model->n; ++j){
        for(ffm_int f = 0; f < model->m; ++f){
            for(ffm_int d = 0; d < model->k; ++d, ++ptr)
                fprintf(fp, "%f ", static_cast<float>(*ptr));
            fprintf(fp, "\n");
        }
    }
    return 0;
...
ffm_model* my_ffm_load_model(char const *path){
    ...
    fscanf(fp, "%s %d %s %d %s %d %s %d", dummy, &model->n, dummy, &model->m, dummy, &model->k, dummy, &model->normalization);
