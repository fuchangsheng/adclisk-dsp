...
var mADOperatorModel = require('../../model/aduser_operators').create();
var mADTemplateModel = require('../../model/adlib_target_template').create();
var mADAssetsModel = require('../../model/adlib_assets').create();
var mRoleModel = require('../../model/role').create();
...
function getUnitInfoById(unit_id, cb) {
    var match = {
        unit_id: unit_id,
    };
    var select = {};
    ...
    mADUnitModel.lookup(query, function(err, rows) {
        if(err) {
            ...
            if(rows.length == 0) {
                var msg = 'Cannot find unit_id ' + unit_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var unit_info = rows[0];
                cb(null, unit_info);
            }
        }
    });
...
function getIdeaInfoById(idea_id, cb) {
    var match = {
        idea_id: idea_id,
    };
    var select = {};
    ...
    mADIdeaModel.lookup(query, function(err, rows) {
        if(err) {
            ...
            if(rows.length == 0) {
                var msg = 'Cannot find idea_id ' + idea_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var idea_info = rows[0];
                cb(null, idea_info);
            }
        }
    });
...
function getOperatorInfoById(oper_id, cb) {
    var match = {
        oper_id: oper_id,
    };
    var select = {};
    ...
    mADOperatorModel.lookup(query, function(err, rows) {
        if(err) {
            cb(err);
        } else {
            if(rows.length == 0) {
                var msg = 'Cannot find oper_id ' + oper_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var oper_info = rows[0];
                cb(null, oper_info);
            }
        }
    });
...
function getTemplateInfoById(template_id, cb) {
    var match = {
    ...
    mADTemplateModel.lookup(query, function(err, rows) {
        if(err) {
            cb(err);
        } else {
            if(rows.length == 0) {
                var msg = 'Cannot find template_id ' + template_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var template_info = rows[0];
                cb(null, template_info);
            }
        }
    });
}

function getAssetInfoById(asset_id, cb) {
    var match = {
        asset_id: asset_id,
    };
    var select = {};
    ...
    mADAssetsModel.lookup(query, function(err, rows) {
        if(err) {
            cb(err);
        } else {
            if(rows.length == 0) {
                var msg = 'Cannot find asset_id ' + asset_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var asset_info = rows[0];
                cb(null, asset_info);
            }
        }
    });
...
function getRoleInfoById(role_id, cb) {
    var match = {
        id: role_id,
    };
    var select = {};
    ...
    mRoleModel.lookup(query, function(err, rows) {
        if(err) {
            cb(err);
        } else {
            if(rows.length == 0) {
                var msg = 'Cannot find role_id ' + role_id;
                mLogger.error(msg);
                cb({code: ERRCODE.DATA_INVALID, msg: msg});
            } else {
                var role_info = rows[0];
                cb(null, role_info);
            }
        }
    });
...
module.exports.getUnitInfoById = getUnitInfoById;
module.exports.getIdeaInfoById = getIdeaInfoById;
module.exports.getOperatorInfoById = getOperatorInfoById;
module.exports.getTemplateInfoById = getTemplateInfoById;
module.exports.getAssetInfoById = getAssetInfoById;
module.exports.getRoleInfoById = getRoleInfoById;
