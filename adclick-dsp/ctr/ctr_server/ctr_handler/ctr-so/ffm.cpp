...
int Ffm::init() {
	...
	model = ffm_load_model(model_path.c_str());
	if (model == NULL) {
		std::cerr << "Fatal Can't open ffm model file" << std::endl;
		return -1;
	}
	return 0;
}

double Ffm::predict(vector<int> vec){
    vector<ffm_node> x;
    ...
    return ffm_predict(x.data(), x.data()+x.size(), model);
}
Ffm::~Ffm(){
    ffm_destroy_model(&model);
}
